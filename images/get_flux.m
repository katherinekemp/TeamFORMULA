...
function totalCharge = get_flux(turns, d_car, turns_car, radius_car, height, spacing, velocity, rho, scenarioID, BSmag, BZ, X, Y, Z, X_M, Y_M, maxDistance, increment, filamentStep, meshDistance, heightIndex, numberOfSquaresX, numberOfSquaresY, outputFolder) 
    %% FORMULA Constants
    ...
    filename1 = sprintf('%s/Scenario%u', outputFolder, scenarioID);
    ...
    for j = round(2 * spacing / increment - 1): -1 : 1
        ...
        if k == round(2 * spacing / increment)
            k = round(4 * spacing / increment - 1);
        else
            k = k - 1;
        end
    ...
    k = round(2 * spacing / increment);
    for j = round(4 * spacing / increment) : length(flux)
        if flux(j) == 0
            flux(j) = flux(k);
        end
        
        if k == round(4 * spacing / increment - 1)
            k = round(2 * spacing / increment);
        else
            k = k + 1;
        end
