...
function data = get_field(V, wireGauge, turns, radius, wireGauge_car, turns_car, radius_car, height, original_spacing, velocity, scenarioID, outputFolder) 
    ...
    for i = 1:length(A)
        totalCharge = get_flux(turns, A(i), B(i), radius_car, height, spacing, C(i), rho, (scenarioID - 1)*length(A) + i, BSmag, BZ, X, Y, Z, X_M, Y_M, maxDistance, increment, filamentStep, meshDistance, heightIndex, numberOfSquaresX, numberOfSquaresY, outputFolder);
