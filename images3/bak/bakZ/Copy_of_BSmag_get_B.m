function [BSmag,X,Y,Z,BX,BY,BZ] = BSmag_get_B(BSmag,X,Y,Z,muRel)
%---------------------------------------------------
%  NAME:      BSmag_get_B.m
...
for nF = 1:BSmag.Nfilament % Loop on each filament
    ...
    for m = 1:size(X,1);
        for n = 1:size(X,2);
            ...
            % Sum
            BX(m,n,p) = BX(m,n,p) + mu*I/4/pi*sum(DBx);
            BY(m,n,p) = BY(m,n,p) + mu*I/4/pi*sum(DBy);
            BZ(m,n,p) = BZ(m,n,p) + mu*I/4/pi*sum(DBz);
